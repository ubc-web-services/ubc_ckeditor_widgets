...
export default class UbcColorBoxBackgroundUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColorBoxBackgroundUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColorBoxBackground');

    editor.ui.componentFactory.add('ubcColorBoxBackgroundOptions', locale => {
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColorBoxBackground';
  let alloptions = backgroundStyles.map(style => {
    return style;
  });
