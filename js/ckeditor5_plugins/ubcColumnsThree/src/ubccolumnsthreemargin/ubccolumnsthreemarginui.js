...
export default class UbcColumnsThreeMarginUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsThreeMarginUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsThreeMargin');

    editor.ui.componentFactory.add('ubcColumnsThreeMarginOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnsthree-margin-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsThreeMargin';
  let alloptions = marginStyles.map(style => {
    return style;
  });
