...
export default class UbcColumnsThreePaddingUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsThreePaddingUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsThreePadding');

    editor.ui.componentFactory.add('ubcColumnsThreePaddingOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnsthree-padding-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsThreePadding';
  let alloptions = paddingStyles.map(style => {
    return style;
  });
