...
export default class UbcColumnsThreeBackgroundUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsThreeBackgroundUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsThreeBackground');

    editor.ui.componentFactory.add('ubcColumnsThreeBackgroundOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnsthree-bgcolor-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsThreeBackground';
  let alloptions = backgroundStyles.map(style => {
    return style;
  });
