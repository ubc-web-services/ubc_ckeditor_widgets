/**
 * @file defines InsertUbcAccordionCommand, which is executed when the ubcAccordion
...
export default class InsertUbcAccordionCommand extends Command {
  execute() {
    ...
    model.change((writer) => {
      ...
      model.insertContent(createUbcAccordion(writer));
    });
  }
...
function createUbcAccordion(writer) {
  // Create instances of the three elements registered with the editor in
  // ubcaccordionediting.js.
  const ubcAccordion = writer.createElement('ubcAccordion');
  const ubcAccordionTitle = writer.createElement('ubcAccordionTitle', { 'aria-expanded': true } );
  const ubcAccordionDescription = writer.createElement('ubcAccordionDescription', { hidden: null });
