...
export default class UbcColumnsTwoBackgroundUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsTwoBackgroundUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsTwoBackground');

    editor.ui.componentFactory.add('ubcColumnsTwoBackgroundOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnstwo-bgcolor-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsTwoBackground';
  let alloptions = backgroundStyles.map(style => {
    return style;
  });
