...
export default class UbcColumnsTwoPaddingUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsTwoPaddingUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsTwoPadding');

    editor.ui.componentFactory.add('ubcColumnsTwoPaddingOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnstwo-padding-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsTwoPadding';
  let alloptions = paddingStyles.map(style => {
    return style;
  });
