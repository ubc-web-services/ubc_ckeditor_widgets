...
export default class UbcColumnsFourGapUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsFourGapUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsFourGap');

    editor.ui.componentFactory.add('ubcColumnsFourGapOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnsfour-gap-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsFourGap';
  let alloptions = gapStyles.map(style => {
    return style;
  });
