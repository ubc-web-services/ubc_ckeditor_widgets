...
export default class UbcColumnsFourPaddingUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsFourPaddingUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsFourPadding');

    editor.ui.componentFactory.add('ubcColumnsFourPaddingOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccolumnsfour-padding-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsFourPadding';
  let alloptions = paddingStyles.map(style => {
    return style;
  });
