...
export default class UbcColumnsFourMarginUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcColumnsFourMarginUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcColumnsFourMargin');

    editor.ui.componentFactory.add('ubcColumnsFourMarginOptions', locale => {
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcColumnsFourMargin';
  let alloptions = marginStyles.map(style => {
    return style;
  });
