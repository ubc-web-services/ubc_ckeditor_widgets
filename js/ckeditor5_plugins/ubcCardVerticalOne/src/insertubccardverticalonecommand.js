/**
 * @file defines InsertUbcCardVerticalOneCommand, which is executed when the ubcCardVerticalOne
...
export default class InsertUbcCardVerticalOneCommand extends Command {
  execute() {
...
function createUbcCardVerticalOne(writer) {
  ...
  const ubcCardVerticalOne = writer.createElement('ubcCardVerticalOne');
  ...
  const ubcCardVerticalOneImagePlaceholder = writer.createElement( 'imageBlock', { src: 'https://placehold.co/700x350' } );
  const ubcCardVerticalOneContent = writer.createElement('ubcCardVerticalOneContent');
  const ubcCardVerticalOneHeading = writer.createElement('heading3', { class: 'card__title' });
  ...
  const ubcCardVerticalOneLink = writer.createText('Learn more', { linkHref: '/' });
