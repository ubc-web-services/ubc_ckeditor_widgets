...
export default class UbcCardVerticalOneBackgroundUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcCardVerticalOneBackgroundUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcCardVerticalOneBackground');

    editor.ui.componentFactory.add('ubcCardVerticalOneBackgroundOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccardverticalone-bgcolor-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcCardVerticalOneBackground';
  let alloptions = backgroundStyles.map(style => {
    return style;
  });
