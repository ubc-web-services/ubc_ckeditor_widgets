...
export default class UbcCardVerticalOneMarginUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcCardVerticalOneMarginUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcCardVerticalOneMargin');

    editor.ui.componentFactory.add('ubcCardVerticalOneMarginOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccardverticalone-margin-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcCardVerticalOneMargin';
  let alloptions = marginStyles.map(style => {
    return style;
  });
