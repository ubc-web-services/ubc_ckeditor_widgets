...
export default class UbcCardVerticalThreeMarginUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'UbcCardVerticalThreeMarginUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('ubcCardVerticalThreeMargin');

    editor.ui.componentFactory.add('ubcCardVerticalThreeMarginOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-ubccardverticalthree-margin-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'ubcCardVerticalThreeMargin';
  let alloptions = marginStyles.map(style => {
    return style;
  });
